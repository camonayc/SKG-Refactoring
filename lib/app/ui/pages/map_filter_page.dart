...
class MapFilterPage extends StatelessWidget {
  final FilterController _controller = FilterController.initializeController();

  MapFilterPage({super.key});
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
              ...
              onPressed: () {
                _controller.showFilter.value = false;
                Get.back();
        ...
        body: Stack(
          children: [
            ...
            // Obx(() => _controller.noIncidents.value == false
            //     ? GestureDetector(
            //         onTap: () {
            //           _controller.noIncidents.value == true;
            //         },
            //         child: Container(
  ...
  Obx markerCardData(FilterController controller, BuildContext context) {
    final size = MediaQuery.of(context).size;
  ...
  String getLocalidad(String? id) {
    return (_controller.localidades.where((p0) => p0.id.toString() == id).first)
        .categoryName;
  }

  String getClase(String? id) {
    return (_controller.clases.where((p0) => p0.id.toString() == id).first)
        .categoryName;
  }
}
